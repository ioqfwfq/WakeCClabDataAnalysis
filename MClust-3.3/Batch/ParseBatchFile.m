...
blanklines = find(strcmp(lines, ''));
lines(blanklines) = [];                      % remove cells with empty strings
...
if isempty(ig_first) | isempty(ig_last)
	error( ['Could not find the BEGINGLOBALS or ENDGLOBALS keyword. These are required exactly once!']);   
end
if length(ig_first)>1 | length(ig_last)>1
	 error( ['There are multiple BEGINGLOBALS or ENDGLOBALS keywords. These are required exactly once!']);   
end
...
for ii = ig_first+1 : ig_last-1
  ...
  case 'CLUSTERALGORITHM'
      ...
      if (strcmpi(Values{ii}{1},'BBClust') | strcmpi(Values{ii}{1},'KlustaKwik'))
        gPar.ClusterAlgorithm = Values{ii}{1};
      else
        error( ['Keyword ' KeyWord{ii} ' takes only  parameters "BBClust"  or "KlustaKwik"!'] );
      end%if
...
for ii = 1:length(gPar.RemoveList)
   ...
   elseif ~isempty(gPar.RemoveList{ii})
      warning(['Could not remove file ' gPar.RemoveList{ii} ' from file list. File not found in list!']);
   end
end%for
...
if isempty(ig_first) | isempty(ig_last)
	error( ['Could not find the BEGINDEFAULTS or ENDDEFAULTS keyword. These are required exactly once!']);   
end
if length(ig_first)>1 | length(ig_last)>1
	 error( ['There are multiple BEGINDEFAULTS or ENDDEFAULTS keywords. These are required exactly once!']);   
end
  ...
  % loop over all files in current --FILE section
   ...
   for ii = ig_first : ig_last
      ...
      case 'CHANNELVALIDITY'
         ...
         for jj=1:4
            fPar{findx}.ChannelValidity(jj) = str2num(Values{ii}{jj});
         end
      ...
      case 'SUBSAMPLEPARAMETERS'
         ...
         elseif length(Values{ii}) > 0  
            for jj=1:length(Values{ii})
               fPar{findx}.SubsampleParameters(jj) = str2num(Values{ii}{jj});
            end
         end
