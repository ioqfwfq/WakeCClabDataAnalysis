function [wavePCData, wavePCNames, wavePCPar] = feature_wavePC3(V, ttChannelValidity, Params)

% MClust
% [Data, Names] = feature_wavePCA(V, ttChannelValidity)
% Calculate first waveform PRINCIPAL COMPONENTS (PC3) 
%
% INPUTS
%    V = TT tsd
...
%%% PARAMETERS:  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
iPC  = 3;    % number of principal component to keep (per channel)
...
case 3
   recalcParams = 0;
otherwise
   error('feature_wavePC3 needs 2 or 3 input arguments');
end
...
for iC = 1:lf
   ...
   wavePCNames{iC} = ['wavePC3'  ': ' num2str(f(iC))];
end
