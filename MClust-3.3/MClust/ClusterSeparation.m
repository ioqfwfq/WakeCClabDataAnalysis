		...
		if n_records <= MaxRecords
			for iF = 1:nFeatures
				...
				if ~exist(FDFileName) 
					...
					NormalizeYN = 'yes';
					disp(' ')
					disp(['FD file ' FDFileName ' does not exist; creating...'])
					disp(' ')
			...
			for iF = 1:nFeatures
				...
				temp = load(FDFileName,'-mat');
				if size(temp.FeatureData,2) == size(ChannelValidity,2)
					FD_f = [FD_f temp.FeatureData(f,find(ChannelValidity))];
				else
			...
			for iB = 1:nBlocks
				...
				for iF = 1:nFeatures
					...
					temp = load(FDFileName,'-mat');
					if size(temp.FeatureData,2) == size(ChannelValidity,2)
						FD = [FD temp.FeatureData(StartRecord:EndRecord,find(ChannelValidity))];
					else
...
IsolationDist = ClusterQuality(FD,f,Dists);
%-------------------------------------------
...
% 	msgstr{end+1} = sprintf('Chi fit (corr)          = %3.2f',C);
	...
	H_In = hist(IntraDist(find(IntraDist < mxDist)),bins);
	H_In = H_In./sum(H_In);
