...
switch action
    ...
    case 'DrawWaveforms'
        ...
        if ~isempty(GP.limits{GP.CurrentChannel})
            ...
            h = line(GP.limits{GP.CurrentChannel}(:,[1 1])',GP.limits{GP.CurrentChannel}(:,[2 3])'); 
            set(h,'LineStyle',':')
            set(h,'Color','w')
        ...
        zoom on
        title([GP.WaveFile ' ' num2str(length(GP.DrawIdx)) ' of ' num2str(length(GP.GoodIdx)) ' Waveforms displayed, ' num2str(size(GP.wv,1)) ' Original Waveforms'])
    ...
    case 'CutOnTimeAndPeak'
        ...
        GP.GoodIdx = GP.GoodIdx(find(PeakVals(:) >= yiB(:) & PeakVals(:) <= yiT(:)));
    ...
    case 'FeaturePlot'
        ...
        if loaded == 1
            ...
            dot_prod = wv./(repmat(energy,1,32))*wave_templates';
    ...
    case 'AddLimit'
        ...
        if min(y)~=max(y) & (min(x) >= 1 | max(x) <= 32)
            ...
            GP.PrevGoodIdx = GP.GoodIdx;
            GP.GoodIdx = GP.GoodIdx(find(GP.wv(GP.GoodIdx,GP.CurrentChannel,x) < max(y) & GP.wv(GP.GoodIdx,GP.CurrentChannel,x) >= min(y)));
            WaveformCutter('DrawWaveforms')
        else
    ...
    case 'DensityPlot'
        ...
        Waveform_density(squeeze(GP.wv(GP.GoodIdx(idx),GP.CurrentChannel,:)),1,2000);
        hold on
        if subsampled
    ...
    case 'CutOnFullHist'
        ...
        if pt >0
            GP.GoodIdx = GP.GoodIdx(find(GP.wv(GP.GoodIdx,GP.CurrentChannel,pt) < max(x) & GP.wv(GP.GoodIdx,GP.CurrentChannel,pt) >= min(x)));
        elseif pt > -9 & pt < 0
            GP.GoodIdx = GP.GoodIdx(find(GP.Params(GP.GoodIdx,abs(pt)) < max(x) & GP.Params(GP.GoodIdx,abs(pt)) >= min(x)));
        elseif pt == -9
            w = squeeze(GP.wv(GP.GoodIdx,GP.CurrentChannel,6:10));
            curvness = sum((w - repmat(mean(w')',1,size(w,2)))'.^2);
            clear w
            pack
            GP.GoodIdx = GP.GoodIdx(find(curvness < max(x) & curvness >= min(x)));
        end
        hold on
        a = axis
        a_color = rand(1,3);
...
uicontrol('Parent', figHandle, ...
    ...
    'Style', 'pushbutton', 'Tag', 'Remove', 'String', 'Remove', 'Callback', 'WaveformCutter(''Remove'')', ...
    'TooltipString', 'Remove the currently selected waveforms from the original record and continue cutting.');
...
for ch = 1:length(GP.limits)
    for lim = 1:size(GP.limits{ch},1)
        if GP.limits{ch}(lim,1) > 0
            GP.GoodIdx = GP.GoodIdx(find(GP.wv(GP.GoodIdx,ch,GP.limits{ch}(lim,1)) < max(GP.limits{ch}(lim,2:3)) & GP.wv(GP.GoodIdx,ch,GP.limits{ch}(lim,1)) >= min(GP.limits{ch}(lim,2:3))));
        else
            GP.GoodIdx = GP.GoodIdx(find(GP.Params(GP.GoodIdx,abs(GP.limits{ch}(lim,1))) < max(GP.limits{ch}(lim,2:3)) & GP.Params(GP.GoodIdx,abs(GP.limits{ch}(abs(lim),1))) >= min(GP.limits{ch}(abs(lim),2:3))));
        end
    end
end
return
