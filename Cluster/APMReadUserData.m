function apmdata = APMReadData( filename )
...
MSG_REWARD					= 17;
MSG_LFP_TIME_CALIBRATION	= 18;   %XQ 2007 feb 4 according to steph
MSG_TEMPLATE				= 32;
MSG_CONTINUOUS				= 48;   %XQ
MSG_LFP     				= 56;   %XQ
...
% Initialization
nchan   = 48;              % Default number of channels
        ...
        if (~feof(fdat))
            ...
            switch (m_code)
                case MSG_START_OF_TRIAL
                    ...
                    trials(ntr(m_channel)).channels(m_channel).lfp_time_calibration=lfptcal(m_channel); % XQ 2007 feb 04
                    for i=1:nunits
                ...
                case MSG_LFP_TIME_CALIBRATION % XQ 2007 feb 4
                    ...
                    if (ntr(m_channel)>0)
                        for i=1:(m_length/2)
                            ...
                            if (ts<last_ts(m_channel))
                                ...
                                trials(ntr(m_channel)).channels(m_channel).voltage_calibration=vcal(m_channel);
                                trials(ntr(m_channel)).channels(m_channel).time_calibration=tcal(m_channel);
                                trials(ntr(m_channel)).channels(m_channel).lfp_time_calibration=lfptcal(m_channel);%XQ
                            end;
                ...
                case MSG_TCPIP_USER256
                    ...
                    try
                        %                         disp(sprintf(' Trial Number : %d ...',double(m_data(1))));
                        tNumber = double(m_data(1));
                        %                         disp(sprintf('  Trial Type  : %d ...',double(m_data(2))));
                ...
                case MSG_TCPIP_USER257
                    ...
                    try
                        %                         disp(sprintf(' Trial Number : %d ...',double(m_data(1))));
                        tNumber = double(m_data(1));
                        %                         disp(sprintf('  Rewarded    : %d ...',double(m_data(2))));
